function [r0, truth, nuclei_per_object] = load_truth(trial_str)
...
% James Kapaldo
    ...
    trutht = truth(:,[2,3,1]);
    r0x = accumarray(trutht(:,3), trutht(:,1),[max(trutht(:,3)),1],@(x) {x});
    r0y = accumarray(trutht(:,3), trutht(:,2), [max(trutht(:,3)),1],@(x) {x});
