...
% modeling.
%
% [r0,Info] = computeInitialPoints(method, BW, B, options)
%
% Create a set of initial positions r0, using METHOD, from the binary mask
% BW and/or the boundaries of the mask B.  OPTIONS should be a structure
% that gives additional information. Each option should be a field with the
% required value. There is one required option for all methods, and some
% methods have additional required options.
%
% Input parameters:
% method - The method used to compute the initial points (see below).
% BW - A binary mask of a single object
% B - An Mx2 array with the boundaries of the object
...
%
% curvatureCenters - An array of the centers of the circles computed from
%                    the boundary curvature. These values are obtained from
...
% Notes:
%
% * The boundaries may have holes, and the holes should be delimited by a
% row of NaNs. The outer boundary should have clockwise orientaiton and the
% holes should have counter-clockwise orientation.
%
...
%
% * No initial point within 3 pixels of a boundary will be given. This
% ensures the particles will not start with very large potential energies.
%
...
if any(strcmp(method,{'curvatureRandom','curvatureUniformRandom'}))
    ...
    if ~isfield(options,'curvatureCenters')
        error('getInitialPoints:missingInput','You must have a field named "curvatureCenters" in OPTIONS that curvature centers.')
    end

    markers = options.curvatureCenters;
